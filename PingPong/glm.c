...
glmUnitize(GLMmodel* model)
{
    ...
    /* calculate model wideltaTimeh, height, and depth */
...
/* glmDimensions: Calculates the dimensions (wideltaTimeh, height, depth) of
 * a model.
 *
 ...
 * values are expected: wideltaTimeh, height of the image and the maximum
 * value for a pixel (max_value must be < 256 for PPM raw files).  The
 * data section consists of wideltaTimeh*height rgb triplets (one byte each)
 ...
 * wideltaTimeh      - will contain the wideltaTimeh of the image on return.
 * height     - will contain the height of the image on return.
 *
 */
GLubyte* 
glmReadPPM(char* filename, int* wideltaTimeh, int* height)
{
    FILE* fp;
    int i, w, h, d;
    unsigned char* image;
    ...
    /* grab the three elements in the header (wideltaTimeh, height, maxval). */
    i = 0;
    while(i < 3) {
        fgets(head, 70, fp);
